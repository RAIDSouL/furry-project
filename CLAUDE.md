...
- **Combat** (started): `player.gd` has an **ATTACK** state — left-click (TPS) swings, locking movement; at the strike frame it sweeps a forward sphere (`intersect_shape`, mask = physics **layer 3** = enemy hurtboxes) and routes damage to the server via `take_damage.rpc_id(1, dmg)` (offline solo runs it locally). **`scenes/dummy.tscn`** (`scripts/dummy.gd`) is a **server-authoritative** training dummy: a `StaticBody3D` with a layer-3 `Hurtbox` (Area3D), a billboarded 3D HP bar, hit flash, and auto-respawn. Only the server (or the local peer when offline) mutates `_health`; it's synced to all peers via a `MultiplayerSynchronizer` (alive state is derived from `_health > 0`, so nothing else needs replicating). Spawned by `gameplay.gd` (server/solo) under a `Monsters` container + `MonsterSpawner`. Combat is **TPS-only** for now (no ISO attack) and uses placeholder animation (freezes like dodge).
