...
- **Multiplayer** (working): 3-player co-op over **ENet, direct IP, client-authoritative** (port 24565). `scripts/network_manager.gd` (autoload `NetworkManager`) is the transport; **`scripts/gameplay.gd`** (on the `Gameplay.tscn` root) drives the Host/Join/Single-Player menu (`NetworkUI`) and spawns a player per peer (server instantiates → `MultiplayerSpawner` replicates). The `MultiplayerSynchronizer` on `player.tscn` syncs position, model rotation, and the anim blend; the default server-relay propagates each client's state to the others. Single-player is just a host nobody joins. See `CHANGELOG.md`.
